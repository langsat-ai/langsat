...
# ══════════════════════════════════════════════════════════════════════════════════════════
# 15 gradient boosting + the feature-engineering SQL plan
# ══════════════════════════════════════════════════════════════════════════════════════════
write("15_gbm_feature_plan", "15_gbm_feature_plan.ipynb", [
    intro("15 · Gradient boosting with a feature-engineering plan",
          "The graph models learn features from the neighbourhood; a gradient-boosted tree wants them written down as columns. Langsat's GBM lane does that with a **feature-engineering SQL plan** — SQL over the cleaned tables that produces one row per entity with engineered columns, written by the model for your task, or by you in the app — and trains LightGBM on it. This chapter reviews the cleaning plan on a training project, has the model write the feature plan, reads the SQL and its safety checks, trains LightGBM, reads importance, and scores a new review with its related rows.",
          ["The model menu with a `gbm` family next to `neural`, and how to pick it (`model_key=\"lightgbm\"`)",
           "The cleaning plan on a `data_science` project (the training pipeline runs it — review it first)",
           "`feature-plan/generate`: the model writes the feature SQL for your task; read the SQL, the columns, the roles, the warnings",
           "What the validator refuses — including a leak the correlation check cannot see",
           "Editing the SQL yourself: the app's Feature Engineering panel; why a key is refused there",
           "Train LightGBM; the engine fields on the model row; feature importance",
           "Predict from a new row: a GBM model scores values, and a plan that joins related tables needs them inline (`related_data`)"],
          "one GBM training run (a few minutes; the estimate is reserved and unused minutes refunded), one feature-plan generation (1,000 credits — a model call, refunded on any failure) and a prediction at 50 credits.",
          lane="LightGBM on a feature-plan table"),
    code(PREAMBLE),
    code('''plan = ls.me()["tier"]
...
print("\\n".join(l for l in saved["compiled_sql"].splitlines() if "@col:brand" in l))'''),
    md("""## 3 · Let the model write the feature plan
...
fig(viz.model_dashboard(p, cols=2))'''),
    code('''imp = p.models.importance(model["model_id"])
